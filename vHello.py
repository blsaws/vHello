...
logger = ft_logger.Logger("vHello").getLogger()
...
# Cloudify parameters
vHello_DIR = (REPO_PATH +
            functest_yaml.get("general").get("directories").get("dir_vHello"))
vHello_DATA_DIR = functest_yaml.get("general").get(
    "directories").get("dir_vHello_data") + "/"
vHello_TEST_DIR = functest_yaml.get("general").get(
    "directories").get("dir_repo_vHello_test") + "/"
DB_URL = functest_yaml.get("results").get("test_db_url")

TENANT_NAME = functest_yaml.get("vHello").get("general").get("tenant_name")
TENANT_DESCRIPTION = functest_yaml.get("vHello").get(
    "general").get("tenant_description")
IMAGES = functest_yaml.get("vHello").get("general").get("images")

CFY_MANAGER_BLUEPRINT = functest_yaml.get(
    "vHello").get("cloudify").get("blueprint")
CFY_MANAGER_REQUIERMENTS = functest_yaml.get(
    "vHello").get("cloudify").get("requierments")
CFY_INPUTS = functest_yaml.get("vHello").get("cloudify").get("inputs")

CW_BLUEPRINT = functest_yaml.get("vHello").get("helloworld").get("blueprint")
CW_DEPLOYMENT_NAME = functest_yaml.get("vHello").get(
    "helloworld").get("deployment-name")
CW_INPUTS = functest_yaml.get("vHello").get("helloworld").get("inputs")
CW_REQUIERMENTS = functest_yaml.get("vHello").get(
    "helloworld").get("requierments")
...
def test_helloworld():
    script = "source " + vHello_DATA_DIR + "venv_cloudify/bin/activate; "
    script += "cd " + vHello_DATA_DIR + "; "
    script += "cfy status | grep -Eo \"([0-9]{1,3}\.){3}[0-9]{1,3}\""
    cmd = "/bin/bash -c '" + script + "'"

    try:
        logger.debug("Trying to get helloworld manager IP ... ")
...
#     logger.info("vHello functional test Start Time:'%s'" % (
...
#        logger.info("vHello functional test duration:'%s'" % duration)
...
#            with open(vHello_TEST_DIR + "temp.json") as f:
#                vHello_test_result = json.load(f)
#            f.close()
#        except:
#            logger.error("Unable to retrieve test results")
#
#        set_result("sig_test", duration, vHello_test_result)

        # success criteria for vHello (for Brahmaputra)
...
#
#        try:
#            os.remove(vHello_TEST_DIR + "temp.json")
#        except:
